...
<div class="large-33">
    <div class="photoItems">
        <a class="popupPhoto page" href="/content/photo/popupPhoto?pID=<?php echo $actor ?>_0_<?php echo $recordID ?>_<?php echo $k ?>">
                    ...
                    <ul class="viewComment_<?php echo $photoID ?>  viewComment dropdown-menu" style="width: 300px">
                        ...
                        if (!empty($comment))
                        {
                            foreach ($comment as $k => $value)
                            {
                                ...
                                $commentID = str_replace(':', '_', $value->recordID);
                                $content = $value->data->content;
                                $published = $value->data->published;
                                $fullName = $user->data->fullName;
//                                $f3 = require('comment.php');
                                $this->inc('mains/comment', 'photo', array('comment' => $value, 'user' => $user));
                            }
                        }
                        ?>
                    </ul>
